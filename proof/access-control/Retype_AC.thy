...
locale retype_region_proofs' =
  retype_region_proofs + constrains s :: "det_ext state" and s' :: "det_ext state"

locale Retype_AC_1 =
  fixes aag :: "'a PAS"
  assumes arch_integrity_machine_state_rest[simp]:
    "arch_integrity_subjects subjects aag activate X st (s\<lparr>machine_state := ms\<lparr>machine_state_rest := r\<rparr>\<rparr>) =
     arch_integrity_subjects subjects aag activate X st (s\<lparr>machine_state := ms\<rparr>)"
...
locale Retype_AC_2 = Retype_AC_1 +
  assumes arch_integrity_kheap_ekheap_update[simp]:
    "\<And>f g. arch_integrity_subjects subjects aag activate X s (kheap_update f (ekheap_update g s))"
  and detype_arch_integrity:
    "\<lbrakk> arch_integrity_subjects subjects aag activate X st s;
       integrity_obj_state aag activate subjects st s;
       integrity_obj_state aag activate subjects s (detype R s) \<rbrakk>
       \<Longrightarrow> arch_integrity_subjects subjects aag activate X st (detype R s)"
  and state_vrefs_detype[simp]:
  ...
  and integrity_irq_state_independent[intro!, simp]:
    "\<And>f. integrity aag X st (s\<lparr>machine_state :=
                                machine_state s\<lparr>irq_state := f (irq_state (machine_state s))\<rparr>\<rparr>) =
          integrity aag X st s"
  and untyped_min_bits_le_2:
    "2 \<le> untyped_min_bits"
  and arch_integrity_work_units_completed_update[simp]:
    "\<And>f. arch_integrity_subjects subjects aag activate X st (work_units_completed_update f s) =
...
lemma detype_integrity:
  ...
  apply (clarsimp simp: integrity_def)
  apply (subst detype_arch_integrity; fastforce simp: detype_def detype_ext_def integrity_def)
  done
...
lemma reset_untyped_cap_integrity:
  ...
  apply (clarsimp simp: aag_cap_auth_def ptr_range_def aag_has_Control_iff_owns
                        pas_refined_refl le_trans[OF untyped_min_bits_le_2])
  done
...
(* FIXME ryanb: move if only used in ArchRetype_AC *)
lemma invs_mdb_cte':
...
lemma decode_untyped_invocation_authorised:
  ...
  apply(auto dest!: bang_0_in_set
              simp: valid_cap_def cap_aligned_def obj_ref_of_def is_cap_simps
                    ...
                    aag_cap_auth_def ptr_range_def le_trans[OF untyped_min_bits_le_2]
