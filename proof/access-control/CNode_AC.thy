...
locale CNode_AC_1 =
  ...
  and state_vrefs_cdt_update[simp]:
    "\<And>f. state_vrefs (cdt_update f s) = state_vrefs s"
  and state_vrefs_is_original_cap_update[simp]:
    "\<And>f. state_vrefs (is_original_cap_update f s) = state_vrefs s"
  and state_vrefs_interrupt_states_update[simp]:
    "\<And>f. state_vrefs (interrupt_states_update f s) = state_vrefs s"
  and state_vrefs_machine_state_update[simp]:
    "\<And>f. state_vrefs (machine_state_update f s) = state_vrefs s"
  and state_vrefs_more_update[simp]:
    "\<And>f. state_vrefs (trans_state f s :: det_ext state) = state_vrefs s"
  and sata_update:
  ...
  and arch_integrity_machine_state:
    "\<lbrakk> \<And>P. \<lbrace>\<lambda>ms. P (underlying_memory ms)\<rbrace> mop \<lbrace>\<lambda>_ ms. P (underlying_memory ms)\<rbrace>;
       \<And>P. \<lbrace>\<lambda>ms. P (device_state ms)\<rbrace> mop \<lbrace>\<lambda>_ ms. P (device_state ms)\<rbrace>;
       ((), ms) \<in> fst (mop (machine_state s));
       arch_integrity_subjects subjects aag activate X st s \<rbrakk>
       \<Longrightarrow> arch_integrity_subjects subjects aag activate X st (s\<lparr>machine_state := ms\<rparr>)"
  ...
  and state_vrefs_aobj_upd:
    "\<lbrakk> ko_at ko p s; a_type ko = AEndpoint \<or> a_type ko = ANTFN; a_type ko = a_type (f (val :: 'b)) \<rbrakk>
  ...
  and set_cap_state_vrefs[wp]:
    "\<And>P. \<lbrace>\<lambda>s :: det_ext state. P (state_vrefs s)\<rbrace> set_cap cap slot \<lbrace>\<lambda>rv s. P (state_vrefs s)\<rbrace>"
  ...
  and cap_insert_ext_extended_list_integ_lift:
    "\<And>Q a b c d e.
     \<lbrakk> \<lbrace>list_integ (cdt_change_allowed aag {pasSubject aag} (cdt st) (tcb_states_of_state st)) st and Q\<rbrace>
       ...
       \<And>P. \<lbrace>\<lambda>s. P (ekheap s)\<rbrace> cap_insert_ext a b c d e \<lbrace>\<lambda>rv s. P (ekheap s)\<rbrace>;
       \<And>P. \<lbrace>\<lambda>s. P (ready_queues s)\<rbrace> cap_insert_ext a b c d e \<lbrace>\<lambda>rv s. P (ready_queues s)\<rbrace> \<rbrakk>
  ...
  and cap_move_ext_list_integ_lift:
    "\<And>Q a b c d.
     \<lbrakk> \<lbrace>list_integ (cdt_change_allowed aag {pasSubject aag} (cdt st) (tcb_states_of_state st)) st and Q\<rbrace>
       ...
       \<lbrace>\<lambda>_. list_integ (cdt_change_allowed aag {pasSubject aag} (cdt st) (tcb_states_of_state st)) st\<rbrace>;
       \<And>P. \<lbrace>\<lambda>s. P (ekheap s)\<rbrace> cap_move_ext a b c d \<lbrace>\<lambda>rv s. P (ekheap s)\<rbrace>;
       \<And>P. \<lbrace>\<lambda>s. P (ready_queues s)\<rbrace> cap_move_ext a b c d \<lbrace>\<lambda>rv s. P (ready_queues s)\<rbrace> \<rbrakk>
  ...
  and cap_swap_ext_extended_list_integ_lift:
    "\<And>Q a b c d.
     \<lbrakk> \<lbrace>list_integ (cdt_change_allowed aag {pasSubject aag} (cdt st) (tcb_states_of_state st)) st and Q\<rbrace>
       ...
       \<lbrace>\<lambda>_. list_integ (cdt_change_allowed aag {pasSubject aag} (cdt st) (tcb_states_of_state st)) st\<rbrace>;
       \<And>P. \<lbrace>\<lambda>s. P (ekheap s)\<rbrace> cap_swap_ext a b c d \<lbrace>\<lambda>rv s. P (ekheap s)\<rbrace>;
       \<And>P. \<lbrace>\<lambda>s. P (ready_queues s)\<rbrace> cap_swap_ext a b c d \<lbrace>\<lambda>rv s. P (ready_queues s)\<rbrace> \<rbrakk>
  ...
  and empty_slot_extended_list_integ_lift:
    "\<And>Q a b.
     \<lbrakk> \<lbrace>list_integ (cdt_change_allowed aag {pasSubject aag} (cdt st) (tcb_states_of_state st)) st and Q\<rbrace>
       empty_slot_ext a b
       \<lbrace>\<lambda>_. list_integ (cdt_change_allowed aag {pasSubject aag} (cdt st) (tcb_states_of_state st)) st\<rbrace>;
       \<And>P. \<lbrace>\<lambda>s. P (ekheap s)\<rbrace> empty_slot_ext a b \<lbrace>\<lambda>rv s. P (ekheap s)\<rbrace>;
       \<And>P. \<lbrace>\<lambda>s. P (ready_queues s)\<rbrace> empty_slot_ext a b \<lbrace>\<lambda>rv s. P (ready_queues s)\<rbrace>\<rbrakk>
  ...
  "\<lbrace>pas_refined aag and
    (\<lambda>s. (is_transferable_in ptr s \<and> (\<not> Option.is_none (cdt s ptr)))
          \<longrightarrow> is_transferable_cap cap \<or>
              (pasObjectAbs aag (fst $ the $ cdt s ptr), Control, pasObjectAbs aag (fst ptr))
...
lemma dmo_no_mem_respects:
  ...
  shows "\<lbrace>integrity aag X st\<rbrace> do_machine_op (mop :: unit machine_monad) \<lbrace>\<lambda>_. integrity aag X st\<rbrace>"
  unfolding do_machine_op_def integrity_def
  by (wpsimp simp: split_def arch_integrity_machine_state[OF p q])
...
crunches set_original
  ...
  and tcb_domain_map_wellformed[wp]: "tcb_domain_map_wellformed aag"
...
lemma set_cdt_pas_refined:
  ...
  shows "\<lbrace>pas_refined aag and (\<lambda>s. \<forall>x y. c x = Some y \<and> cdt s x \<noteq> Some y
           \<longrightarrow> (is_transferable (caps_of_state s x) \<or>
               (pasObjectAbs aag (fst y), Control, pasObjectAbs aag (fst x)) \<in> pasPolicy aag) \<and>
               (pasObjectAbs aag (fst y), DeleteDerived, pasObjectAbs aag (fst x)) \<in> pasPolicy aag)\<rbrace>
         set_cdt c
         \<lbrace>\<lambda>rv. pas_refined aag\<rbrace>"
...
lemma sts_thread_states[wp]:
  "\<lbrace>\<lambda>s. P ((thread_states s)(t := tcb_st_to_auth st))\<rbrace>
   ...
   \<lbrace>\<lambda>rv s. P (thread_states s)\<rbrace>"
  apply (simp add: set_thread_state_def)
  apply (wpsimp wp: set_object_wp dxo_wp_weak)
...
lemma sts_st_vrefs[wp]:
  "\<lbrace>\<lambda>s :: det_ext state. P (state_vrefs s)\<rbrace> set_thread_state t st \<lbrace>\<lambda>rv s. P (state_vrefs s)\<rbrace>"
...
lemma set_simple_ko_vrefs[wp]:
  "\<lbrace>\<lambda>s :: det_ext state. P (state_vrefs s)\<rbrace>
   set_simple_ko f ptr (val :: 'b)
   \<lbrace>\<lambda>rv s. P (state_vrefs s)\<rbrace>"
  apply (simp add: set_simple_ko_def set_object_def)
  apply (wp get_object_wp)
  apply (fastforce simp: state_vrefs_aobj_upd obj_at_def elim!: rsubst[where P=P, OF _ ext])
  done

lemma set_simple_ko_pas_refined[wp]:
  "\<lbrace>\<lambda>s. pas_refined aag s\<rbrace> set_simple_ko f ptr (ep :: 'b) \<lbrace>\<lambda>rv. pas_refined aag\<rbrace>"
  apply (simp add: pas_refined_def state_objs_to_policy_def)
  apply (rule hoare_pre)
   apply (wp tcb_domain_map_wellformed_lift | wps)+
  apply simp
...
lemma thread_set_st_vrefs[wp]:
  "\<lbrace>\<lambda>s :: det_ext state. P (state_vrefs s)\<rbrace> thread_set f t \<lbrace>\<lambda>rv s. P (state_vrefs s)\<rbrace>"
...
locale CNode_AC_4 = CNode_AC_3 +
  ...
  and cap_asid'_cap_rights_update[simp]:
  ...
  and arch_derive_cap_obj_refs_auth:
    "\<lbrace>K (\<forall>r\<in>obj_refs (cap.ArchObjectCap cap).
         \<forall>auth\<in>cap_auth_conferred (cap.ArchObjectCap cap). aag_has_auth_to aag auth r)\<rbrace>
     arch_derive_cap cap
...
context CNode_AC_4 begin

lemma derive_cap_auth_derived:
  "\<lbrace>\<lambda>s :: det_ext state. cap \<noteq> cap.NullCap \<longrightarrow> cte_wp_at (auth_derived cap) src_slot s\<rbrace>
...
lemma decode_cnode_invocation_auth_derived:
  "\<lbrace>\<lambda>s :: det_ext state. True\<rbrace>
   decode_cnode_invocation label args cap excaps
...
(* FIXME: replace ArchIpc_AI.update_cap_data_closedform with the following *)
lemma update_cap_data_closedform:
  "update_cap_data pres w cap =
   (case cap of
     EndpointCap r badge rights \<Rightarrow>
       if badge = 0 \<and> \<not> pres then (cap.EndpointCap r (w && mask badge_bits) rights) else NullCap
   | NotificationCap r badge rights \<Rightarrow>
       if badge = 0 \<and> \<not> pres then (cap.NotificationCap r (w && mask badge_bits) rights) else NullCap
   ...
   | ThreadCap r \<Rightarrow> cap.ThreadCap r
   | DomainCap \<Rightarrow> cap.DomainCap
   | UntypedCap d p n idx \<Rightarrow> cap.UntypedCap d p n idx
   | NullCap \<Rightarrow> cap.NullCap
   | ReplyCap t m rights \<Rightarrow> cap.ReplyCap t m rights
   | IRQControlCap \<Rightarrow> cap.IRQControlCap
   | IRQHandlerCap irq \<Rightarrow> cap.IRQHandlerCap irq
   | Zombie r b n \<Rightarrow> cap.Zombie r b n
