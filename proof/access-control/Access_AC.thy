...
locale Access_AC_1 =
  ...
  and arch_tro_alt_trans_spec:
    "\<lbrakk> arch_integrity_obj_alt aag subjects l ko ko';
       ...
       \<Longrightarrow> arch_integrity_obj_alt aag subjects l ko ko''"
  and arch_integrity_update_autarch:
...
lemma tsos_tro_running:
  "\<lbrakk> \<forall>x. integrity_obj aag activate subjects (pasObjectAbs aag x) (kheap s x) (kheap s' x);
     tcb_states_of_state s p = Some Structures_A.Running; pasObjectAbs aag p \<notin> subjects \<rbrakk>
...
locale Access_AC_2 = Access_AC_1 +
  assumes arch_integrity_trans:
    "\<lbrakk> arch_integrity_subjects subjects aag activate X s s';
       arch_integrity_subjects subjects aag activate X s' s'';
       integrity_obj_state aag activate subjects s s';
       integrity_obj_state aag activate subjects s' s''\<rbrakk>
       \<Longrightarrow> arch_integrity_subjects subjects aag activate X s s''"
  and arch_integrity_refl[simp]:
    "arch_integrity_subjects S aag activate X s s"
  and arch_eintegrity_sa_update:
...
proof -
  show ?thesis using tro_trans t1 t2
    apply (clarsimp simp add: integrity_subjects_def arch_integrity_trans
...
lemma integrity_update_autarch:
  ...
  unfolding integrity_subjects_def
  by (clarsimp simp: arch_integrity_update_autarch)
...
lemma eintegrity_sa_update[simp]:
  ...
  by (fastforce simp: integrity_subjects_def arch_eintegrity_sa_update)
...
lemma pas_refined_all_children':
  ...
  apply (erule disjE)
   ...
   apply (rule disjCI)
   apply (subgoal_tac "(pasObjectAbs aag (fst p), Control, pasObjectAbs aag (fst c)) \<in> pasPolicy aag")
...
lemma ep_rcv_queued_st_tcb_at:
  "\<And>P. \<lbrakk> ko_at (Endpoint ep) epptr s; (t, EPRecv) \<in> ep_q_refs_of ep;
         valid_objs s; sym_refs (state_refs_of s);
         \<And>pl. P (Structures_A.BlockedOnReceive epptr pl); kheap s' t = kheap s t\<rbrakk>
         \<Longrightarrow> st_tcb_at P t s'"
  apply (case_tac ep, simp_all)
...
locale Access_AC_3 = Access_AC_2 +
  ...
  and arch_integrity_obj_atomic_mono:
  ...
  and machine_state_update_state_vrefs[simp]:
    "state_vrefs (machine_state_update f s'') = state_vrefs (s'' :: det_ext state)"
...
lemma integrity_mono:
  ...
  apply (rule conjI)
   apply (clarsimp simp: integrity_ready_queues_def)
   apply blast
  apply (simp add: arch_integrity_mono)
  done
...
lemma state_objs_to_policy_irq_state_independent[intro!, simp]:
  "state_objs_to_policy (s\<lparr>machine_state :=
   ...
   = state_objs_to_policy (s :: det_ext state)"
