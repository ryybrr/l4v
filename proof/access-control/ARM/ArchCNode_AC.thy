...
lemma state_vrefs_cdt_update[CNode_AC_assms, simp]:
  "state_vrefs (cdt_update f s) = state_vrefs s"
  by (simp add: state_vrefs_def)

lemma state_vrefs_is_original_cap_update[CNode_AC_assms, simp]:
  "state_vrefs (is_original_cap_update f s) = state_vrefs s"
  by (simp add: state_vrefs_def)

lemma state_vrefs_interrupt_states_update[CNode_AC_assms, simp]:
  "state_vrefs (interrupt_states_update f s) = state_vrefs s"
  by (simp add: state_vrefs_def)

lemma state_vrefs_machine_state_update[CNode_AC_assms, simp]:
  "state_vrefs (machine_state_update f s) = state_vrefs s"
  by (simp add: state_vrefs_def)

lemma state_vrefs_more_update[CNode_AC_assms, simp]:
...
(* FIXME AC_SPLIT: move to AInvs *)
locale Arch_is_extended = is_extended + Arch
sublocale Arch \<subseteq> cap_swap_extended: Arch_is_extended "cap_swap_ext a b c d" ..
sublocale Arch \<subseteq> cap_move_extended: Arch_is_extended "cap_move_ext a b c d" ..
sublocale Arch \<subseteq> create_cap_extended: Arch_is_extended "create_cap_ext a b c" ..
sublocale Arch \<subseteq> cap_insert_extended: Arch_is_extended "cap_insert_ext a b c d e" ..
sublocale Arch \<subseteq> possible_switch_to_extended: Arch_is_extended "possible_switch_to a" ..
sublocale Arch \<subseteq> tcb_sched_action_extended: Arch_is_extended "tcb_sched_action a b" ..
sublocale Arch \<subseteq> empty_slot_extended: Arch_is_extended "empty_slot_ext a b" ..
sublocale Arch \<subseteq> next_domain_extended: Arch_is_extended "next_domain" ..
sublocale Arch \<subseteq> timer_tick_extended: Arch_is_extended "timer_tick" ..


context Arch_is_extended begin
