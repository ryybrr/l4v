...
lemma arch_integrity_machine_state_rest[Retype_AC_assms, simp]:
  "arch_integrity_subjects subjects aag activate X st (s\<lparr>machine_state := s'\<lparr>machine_state_rest := r\<rparr>\<rparr>) =
   arch_integrity_subjects subjects aag activate X st (s\<lparr>machine_state := s'\<rparr>)"
  by simp
...
(* FIXME ryanb: generic alternative? *)
lemma arch_integrity_kheap_ekheap_update[Retype_AC_assms, simp]:
  "arch_integrity_subjects subjects aag activate X s (kheap_update f (ekheap_update g s))"
  by simp
...
lemma detype_arch_integrity[Retype_AC_assms]:
  "\<lbrakk> arch_integrity_subjects subjects aag activate X st s;
     integrity_obj_state aag activate subjects st s;
     integrity_obj_state aag activate subjects s (detype refs s) \<rbrakk>
     \<Longrightarrow> arch_integrity_subjects subjects aag activate X st (detype refs s)"
  apply (erule arch_integrity_trans)
  apply (auto simp: detype_def detype_ext_def)
  done
...
lemma integrity_irq_state_independent[Retype_AC_assms, intro!, simp]:
...
lemma untyped_min_bits_le_2[Retype_AC_assms]: "2 \<le> untyped_min_bits"
  by (simp add: untyped_min_bits_def)

lemma arch_integrity_work_units_completed_update[Retype_AC_assms, simp]:
  "arch_integrity_subjects subjects aag activate X st (work_units_completed_update f s) =
...
lemma storeWord_respects:
  "\<lbrace>\<lambda>ms. integrity aag X st (s\<lparr>machine_state := ms\<rparr>) \<and>
         (\<forall>p' \<in> ptr_range p 2. aag_has_auth_to aag Write p')\<rbrace>
   storeWord p v
   \<lbrace>\<lambda>a b. integrity aag X st (s\<lparr>machine_state := b\<rparr>)\<rbrace>"
  unfolding storeWord_def
  apply wp
...
lemma dmo_cleanCacheRange_PoU_respects [wp]:
  ...
  by (wpsimp wp: dmo_cacheRangeOp_lift dmo_mol_respects
           simp: cleanCacheRange_PoU_def cleanByVA_PoU_def)
...
global_interpretation Retype_AC_2?: Retype_AC_2
proof goal_cases
  interpret Arch .
  case 1 show ?case
    by (unfold_locales; (fact Retype_AC_assms)?)
