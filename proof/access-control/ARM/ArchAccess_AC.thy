...
named_theorems Access_AC_1_assms

lemma acap_class_reply[Access_AC_1_assms]:
  "(acap_class acap \<noteq> ReplyClass t)"
  (* warning: arch split *)
  by (cases acap; simp)

lemma arch_troa_tro_alt[Access_AC_1_assms, elim!]:
...
lemma clas_caps_of_state[Access_AC_1_assms]:
...
lemma arch_tro_alt_trans_spec[Access_AC_1_assms]:
...
lemma arch_integrity_update_autarch[Access_AC_1_assms]:
...
lemma as_user_state_vrefs[Access_AC_1_assms, wp]:
...
proof goal_cases
  interpret Arch .
  case 1 show ?case
    by (unfold_locales; fact Access_AC_1_assms)
qed


context Arch begin global_naming ARM_A

named_theorems Access_AC_2_assms

lemma arch_eintegrity_sa_update[Access_AC_2_assms]:
...
lemma arch_integrity_trans[Access_AC_2_assms]:
...
lemma arch_integrity_refl[Access_AC_2_assms, simp]:
...
proof goal_cases
  interpret Arch .
  case 1 show ?case
    by (unfold_locales; fact Access_AC_2_assms)
qed


context Arch begin global_naming ARM_A

named_theorems Access_AC_3_assms
...
lemma arch_integrity_mono[Access_AC_3_assms]:
...
lemma arch_integrity_obj_atomic_mono[Access_AC_3_assms]:
...
lemma state_vrefs[Access_AC_3_assms, iff]: "state_vrefs (f s) = state_vrefs s"
...
proof goal_cases
  interpret Arch .
  case 1 show ?case
    by (unfold_locales; fact Access_AC_3_assms)
qed
