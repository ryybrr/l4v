...
lemma arch_integrity_ready_queues_update[Finalise_AC_assms, simp]:
  "arch_integrity_subjects subjects aag activate X st (ready_queues_update f s) =
   arch_integrity_subjects subjects aag activate X st s"
  by simp

lemma arch_integrity_irq_state_update[Finalise_AC_assms, simp]:
  "arch_integrity_subjects subjects aag activate X st
     (s\<lparr>machine_state := machine_state s\<lparr>irq_state := f (irq_state (machine_state s))\<rparr>\<rparr>) =
   arch_integrity_subjects subjects aag activate X st s"
  by simp

lemma arch_integrity_ekheap_update[Finalise_AC_assms, simp]:
  "arch_integrity_subjects subjects aag activate X st (ekheap_update f s) =
   arch_integrity_subjects subjects aag activate X st s"
  by clarsimp
...
crunch pas_refined[wp, wp_not_transferable]: set_vm_root "pas_refined aag"
...
lemma arch_finalise_cap_obj_refs[Finalise_AC_assms]:
  "\<lbrace>\<lambda>s. \<forall>x \<in> aobj_ref' acap. P x\<rbrace> arch_finalise_cap acap slot \<lbrace>\<lambda>rv s. \<forall>x \<in> obj_refs (fst rv). P x\<rbrace>"
  by (wpsimp simp: arch_finalise_cap_def)

(* FIXME ryanb *)
lemma prepare_thread_delete_st_tcb_at_halted[wp]:
  "prepare_thread_delete t \<lbrace>st_tcb_at halted t\<rbrace>"
  by wp
...
proof goal_cases
  interpret Arch .
  case 1 show ?case
    by (unfold_locales; (fact Finalise_AC_assms | wp))
qed
...
global_interpretation Finalise_AC_3?: Finalise_AC_3
proof goal_cases
  interpret Arch .
  case 1 show ?case
