...
locale Interrupt_AC_1 =
  ...
  "\<lbrace>pas_refined (aag :: 'a PAS) and valid_mdb and arch_irq_control_inv_valid irq_ctl_inv and
...
and  arch_invoke_irq_control_respects:
 ...
 arch_invoke_irq_control acinv \<lbrace>\<lambda>y. integrity aag X st\<rbrace>"
...
and arch_check_irq_inv[wp]: "arch_check_irq irq \<lbrace>\<lambda>s :: det_ext state. P s\<rbrace>"
...
lemma invoke_irq_handler_respects:
  "\<lbrace>integrity (aag :: 'a PAS) X st and pas_refined aag and einvs and
  ...
  "\<lbrace>pas_refined aag
      and K (is_subject aag (fst slot)
              ...
              \<and> (args \<noteq> [] \<longrightarrow>
                  (pasSubject aag, Control, pasIRQAbs aag (ucast (args ! 0))) \<in> pasPolicy aag))\<rbrace> arch_decode_irq_control_invocation info_label args slot caps \<lbrace>\<lambda>x s. arch_authorised_irq_ctl_inv aag x\<rbrace>, -"

context Interrupt_AC_2 begin
...
lemma decode_irq_control_invocation_authorised [wp]:
  ...
  apply (rule hoare_pre)
   ...
   apply (wp arch_decode_irq_control_invocation_authorised whenE_throwError_wp hoare_vcg_imp_lift hoare_drop_imps
